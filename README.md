...
## 8) Deploy on AWS (Using Your Credits)

Use this if you want a hosted version accessible over the internet.

### Recommended instance sizes

- Budget start: `t3.large` (2 vCPU, 8 GB)
- Better processing: `c6i.2xlarge` (8 vCPU, 16 GB)

### Step A: Launch EC2

1. Launch Ubuntu 22.04 EC2.
2. In Security Group, allow inbound:
  - `22` (SSH) from your IP
  - `8501` (TCP) from your IP (or your team IP range)
...
```powershell
scp -i <key.pem> -r d:\Sorter ubuntu@<EC2_PUBLIC_IP>:/opt/senior-sorter
```

### Step C: Install Docker and run app

SSH into EC2:

```bash
ssh -i <key.pem> ubuntu@<EC2_PUBLIC_IP>
cd /opt/senior-sorter
chmod +x deploy/aws/ec2_setup.sh deploy/aws/run_container.sh
./deploy/aws/ec2_setup.sh
./deploy/aws/run_container.sh
```
...
### Step D: OAuth note for cloud hosting

Current app login uses desktop-style `run_local_server` flow. That works reliably for local usage, but is not ideal for true multi-user hosted web login.

For real hosted multi-user login, migrate auth to web redirect flow and use a hosted callback URL.

Until then, treat cloud deployment as a single-operator setup.

### Notes

- Keep `client_secret.json` on server and do not commit it to git.
- `output/` is mounted from host so results persist across container restarts.
- If processing is slow, increase EC2 CPU size first before moving to GPU.
